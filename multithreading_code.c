...
char** allocate_words ();
void deallocate_words ();
void readFile ();
void find_top_words (MinHeap myHeap);
cell create_cell (char* word, int frequency);
void count_frequency (int start, int end);
void free_counted_words ();
//heap
...
///data
int n_threads;
char**words; //data is shared among processes
cell* counted_words;
int *arr;

pthread_mutex_t mutex;
void* runner(void* parameter); //threads call this function

int main(int argc, char* argv[])
{
	//allocate  array cells to store words read from file
	words= allocate_words ();

	//create an array of structs (words + frequency), to find words frequency
	counted_words= (cell*)malloc(MAX_SIZE*sizeof(struct result_cell));
	...
	pthread_t myThreads[n_threads]; //thread id
	...
	for (int i=0; i<n_threads; i++){
    	...
    	*thread_num=i;
    	pthread_create (&myThreads[i], &attr, runner, thread_num);
    	//printf("thread #%d started\n",i);
	}

	for (int i=0; i<n_threads; i++){
    	pthread_join(myThreads[i],NULL); //wait for thread to exit
    	//printf("thread #%d finished\n", i);
	}

	//after all threads have finished their execution, find top 10 frequent words
	MinHeap myHeap= createHeap(TOP);
	find_top_words (myHeap);

	print_heap(myHeap);//final results
...
void* runner(void* parameter){
	...
	//Each thread will execute in different data slot

	//start & end are identifires for each thread to determine what section of data must deal with
	int start= (WORDS_NUM*thread_num)/n_threads;
	int end= (WORDS_NUM*(thread_num+1))/n_threads;

	count_frequency(start,end);//supported by Mutex mechanism
	free(parameter);
	...
	for (int i=start; i<end; i++){
    	...
    	for (int j=0; j<frequent_words; j++){
        	if (strcasecmp(words[i], counted_words[j]->word) ==0){
            	...
            	pthread_mutex_lock(&mutex);//lock to avoid race condition
            	(counted_words[j]->frequency)++;
            	pthread_mutex_unlock(&mutex);//lock to avoid race condition
